var GDragonRubyGameId = 'yard-docsify-dragonruby-live';
var GDragonRubyGameTitle = 'Live Demo';
var GDragonRubyDevTitle = 'pvande';
var GDragonRubyGameVersion = '0.1';
var GDragonRubyIcon = '/metadata/icon.png';
var GDragonRubyWriteDir = '/yard-docsify-dragonruby-live';
...
function syncDataFiles(dbname, baseurl) {
  ...
  dbopen.onsuccess = function(event) {
    ...
    xhr.open("get", "dragonruby/manifest.json" + urlrandomizerarg, true);
    xhr.send();
  };
...
var prepareFilesystem = function() {
  ...
  FS.syncfs(true, function(err) {
    ...
    loadDataFiles(GDragonRubyGameId, 'dragonruby/gamedata/', function() {
...
window.parent.window.gtk.load = (text) => {
  if (!window.FS) return;
  FS.writeFile("app/main.rb", text);
  window.parent.window.gtk.exec("$gtk.reset");
  Module.clickToPlayListener();
}

window.parent.window.gtk.hotload = function(text) {
  if (!window.FS) return;
  FS.writeFile("app/main.rb", text);
  Module.clickToPlayListener();
}

window.parent.window.gtk.exec = function(command) {
  if (!window.FS) return;
  FS.writeFile("app/repl.rb", command);
}
...
// document.addEventListener("DOMContentLoaded", loadMainModule);

const COI = Promise[crossOriginIsolated ? 'resolve' : 'reject']();
COI.then(loadMainModule).catch(() => { });
...
//             console.log("One moment, reloading to enable SharedArrayBuffer.");
//             top.location.reload();
//           } else {
//             console.log("Loading…");
//             loadMainModule();
