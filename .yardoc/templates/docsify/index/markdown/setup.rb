...
def init
  super
  sections :index, [:page_head, :page_foot]
end

def docsify_config
  config = JSON.parse(file("docsify.json"))

  overrides = options.files.find { | x | x.filename == "docsify.json" }
  config = config.merge(JSON.parse(overrides.contents)) if overrides
